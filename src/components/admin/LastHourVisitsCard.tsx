    ...
    <Card>
      ...
      {items.length === 0 ? (
        <div className="text-center py-8">
          <Eye className="h-8 w-8 text-gray-300 dark:text-gray-600 mx-auto mb-2" />
          <p className="text-sm text-gray-400">لا توجد زيارات في آخر ساعة</p>
        </div>
      ) : (
          ...
          {items.map((it) => (
            <div key={it.path} className="min-w-0">
              <div className="flex items-center gap-2">
                <p className="flex-1 text-sm text-gray-800 dark:text-gray-200 truncate" title={it.title}>{it.title}</p>
